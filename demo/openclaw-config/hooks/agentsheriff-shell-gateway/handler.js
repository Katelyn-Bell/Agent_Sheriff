...
export default async function agentsheriffShellGateway(event, ctx) {
  const toolName = String(event?.toolName ?? "").toLowerCase();
  if (!isShellToolName(toolName)) {
    return {};
  }
  if (process.env.AGENTSHERIFF_OPENCLAW_SHELL_HOOK_DISABLE === "1") {
    return {};
  }
  ...
  if (command.includes("agentsheriff-shell")) {
    return {};
  }
